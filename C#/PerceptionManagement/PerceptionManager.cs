...
namespace PerceptionManagement
{
    public class PerceptionManager
    {
        ...
        int perceptionDisplayCounter = 0;
        public void OnPhysicalRobotPositionReceived(object sender, LocationArgs e)
        {
            if (robotId == e.RobotId)
            {
                ...
                //On génère la perception
                //if (perceptionDisplayCounter++ %100==0)
                    GeneratePerception();
        ...
        public void OnLidarObjectsReceived(object sender, LidarDetectedObjectListArgs e)
        {
            if (robotPerception.robotKalmanLocation != null)
            {
                lock (physicalObjectList)
                {
                    ...
                    foreach (var obj in e.LidarObjectList)
                    {
                        double angle = obj.AngleMoyen;
                        double distance = obj.DistanceMoyenne;
