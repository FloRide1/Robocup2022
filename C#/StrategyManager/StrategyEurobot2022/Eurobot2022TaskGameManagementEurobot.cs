...
namespace StrategyManagerNS
{
    ...
    public class Eurobot2022TaskGameManagement : TaskBase
    {
                                ...
                                else
                                {
                                    ...
                                    if (newMissionRequested)
                                    {
                                        ...
                                        lock (p.matchDescriptor.listElementsJeu)
                                        {
                                            ...
                                            {
                                                dictionaryElementsRestants = p.matchDescriptor.listElementsJeu.Where(x => x.Value.isAvailable == true &&
                                                x.Value.ReservationToTeam != Eurobot2022TeamReservation.ReservedBlue && x.Value.RobotAttributionYellow == p.robotType)
                                        ...
                                        if (p.matchDescriptor.CompteurDeposeAEffectuer > 0)
                                        {
                                            ...
                                            Dictionary<int, Eurobot2022EmplacementDepose> dictionaryEmplacementsUtilisables = new Dictionary<int, Eurobot2022EmplacementDepose>();
        ...
        void SelectBestStrategy(Dictionary<int, Eurobot2022Gobelet> dictGobeletsRestants, Dictionary<int, Eurobot2022EmplacementDepose> dictEmplacementsUtilisables, Dictionary<int , Eurobot2022StateBrasTurbineRobot> dictBrasVides, Dictionary<int, Eurobot2022StateBrasTurbineRobot> dictBrasAvecGobelets)
        {

        }

        Dictionary<int, Eurobot2022EmplacementDepose> GetListEmplacementsAntecedentsValides(Dictionary<int, Eurobot2022EmplacementDepose> dictionaryEmplacementsRestants)
        {
            Dictionary<int, Eurobot2022EmplacementDepose> dictionaryEmplacementUtilisables = new Dictionary<int, Eurobot2022EmplacementDepose>();
            if (parent != null)
            {
                StrategyEurobot2022 p = parent as StrategyEurobot2022;
        ...
        void CalculateScore()
        {
            ...
            var dictionaryGobeletsRougeDeposes = p.matchDescriptor.DictionaryEmplacementDepose.Where(x => x.Value.IsAvailable == false && x.Value.Color == Eurobot2022Color.Rouge).ToDictionary(x => x.Key, y => y.Value);
            /// On récupère le nombre de gobelets déposés dans un emplacement neutre
            var dictionaryGobeletsNeutreDeposes = p.matchDescriptor.DictionaryEmplacementDepose.Where(x => x.Value.IsAvailable == false && x.Value.Color == Eurobot2022Color.Neutre).ToDictionary(x => x.Key, y => y.Value);
