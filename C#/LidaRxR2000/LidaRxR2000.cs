...
namespace LidaRxR2000NS
{
    public class LidaRxR2000
    {
        ...
        private void LidarStartAndAcquire(double freq, R2000SamplingRate samplingRate)
        {
            while (true)
            {
                ...
                if (!isLidarConnected)
                {
                    ...
                    try
                    {
                        r2000 = new R2000Scanner(IPAddress.Parse("169.254.235.44"), R2000ConnectionType.TCPConnection);
        ...
        public virtual void OnLidarDecodedFrame(int id, List<PolarPointRssi> ptList, int lidarFrameNumber =0)
        {
            ...
            if (handler != null)
            {
                handler(this, new RawLidarArgs { RobotId = id, PtList = ptList, LidarFrameNumber=lidarFrameNumber, Type=LidarDataType.RawPtsList});
            }
        }
    }
}
